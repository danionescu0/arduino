...
void upload(camera_fb_t *fb) {      
    esp_http_client_handle_t httpClient = esp_http_client_init(&httpConfig);
    esp_http_client_set_post_field(httpClient, (const char *)fb->buf, fb->len);
    esp_http_client_set_method(httpClient, HTTP_METHOD_POST);
    esp_http_client_set_header(httpClient, "Content-type", "application/octet-stream");
    esp_err_t clientError = esp_http_client_perform(httpClient);
    if (clientError == ESP_OK)
